...
import 'package:odoosaleapp/helpers/FlushBar.dart'; // Add this import
...
class _AppDrawerState extends State<AppDrawer> {
  ...
  bool _showDeleteAccountButton = false;

  @override
  void initState() {
    super.initState();
    _checkDeleteAccountButtonVisibility();
  }

  Future<void> _checkDeleteAccountButtonVisibility() async {
    final showBtn = await SessionManager().deleteAccountBtn;
    setState(() {
      _showDeleteAccountButton = showBtn ?? false; // Default to false if null
    });
  }
    ...
    if (confirmed == true) {
      ...
      try {
        ...
        if (response != null && response['Control'] == 1) {
          ...
          if (context.mounted) {
            showCustomErrorToast(context, Strings.logout);
          }
        }
      } catch (e) {
        if (context.mounted) {
          showCustomErrorToast(context, 'Error: $e');
        }
      }
    }
  }
  ...
  Future<void> _deleteAccount(BuildContext context) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () => Navigator.pop(context, true),
            child: Text(Strings.deleteAccount, style: const TextStyle(color: Colors.red)),
          ),
        ],
      ),
    );
    ...
    if (confirmed == true) {
      ...
      final customerId = SessionManager().customerId ?? 0;
      final sessionId = SessionManager().sessionId ?? '';
      final email = SessionManager().userName ?? ''; // E-posta adresini al

      try {
        final response = await userService.deleteUser(sessionId,customerId);

        if (response != null && response['Control'] == 1) {
          if (context.mounted) {
            // Başarılı silme sonrası çıkış yap ve login sayfasına yönlendir
            Navigator.pushAndRemoveUntil(
              context,
              MaterialPageRoute(builder: (context) =>  LoginScreen()),
                  (route) => false,
            );
            showCustomToast(context, Strings.deleteAccountSuccess);
          }
        } else {
          if (context.mounted) {
            // API'den gelen hata mesajı varsa onu kullan
            showCustomErrorToast(context, response?['Error'] ?? Strings.deleteAccountFailed);
          }
        }
      } catch (e) {
        if (context.mounted) {
        ...
        children: [
          ...
          ListTile(
            title: InkWell(
              ...
              child: Row(
                children: [
                  Image.network('https://flagcdn.com/w20/nl.png', width: 24, height: 24),
          ...
          ListTile(
            title: InkWell(
              ...
              child: Row(
                children: [
                  Image.network('https://flagcdn.com/w20/gb.png', width: 24, height: 24),
                  const SizedBox(width: 8),
                  const Text('English'),
                  const Spacer(),
                  Radio<AppLanguage>(
          ...
          ListTile(
            title: InkWell(
              ...
              child: Row(
                children: [
                  Image.network('https://flagcdn.com/w20/fr.png', width: 24, height: 24),
                  const SizedBox(width: 8),
                  const Text('Français'),
                  const Spacer(),
                  Radio<AppLanguage>(
          ...
          ListTile(
            title: InkWell(
              ...
              child: Row(
                children: [
                  Image.network('https://flagcdn.com/w20/tr.png', width: 24, height: 24),
                  const SizedBox(width: 8),
                  const Text('Türkçe'),
                  const Spacer(),
                  Radio<AppLanguage>(
          ...
          // Hesabımı Sil butonu, sadece setlenmiş değer true ise görünür
